...
//로딩 시작---------------------------------------------------
        ...
        BPM = firstBPM;
        Time.Start();
        StartCoroutine(judge.JudgeUpdate());
    }
    void Update(){
        if(STOPED){
        ...
        for(int i = 0; notes.Count > i;){
            while(notes.Count > i && currentbeat+1 > notes[i].noteBeat){
                ...
                if(noteType.Equals(1)){
                    BGMs.Enqueue(new BGnotes{noteTime=noteTime, noteSnd=int.Parse(notes[i].noteValues)});
    ...
    IEnumerator playBGM(){
        ...
        while (BGMs.Count > 0){
            ...
            if(currentBGM.noteTime*1000<=Time.Elapsed.TotalMilliseconds){
                ...
                FMODUnity.RuntimeManager.CoreSystem.playSound(WAV[currentBGM.noteSnd], channelGroup, false, out channel[currentBGM.noteSnd]);
                if(BGMs.Count <= 0){
                    break;
                }
                currentBGM = BGMs.Peek();
